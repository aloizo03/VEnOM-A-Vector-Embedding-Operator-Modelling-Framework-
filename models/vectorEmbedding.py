...
from models.utils import get_Transformer_Layer
import numpy as np
import torch
...
class Embedding_Transformer_Layers(nn.Module):
    def __init__(self, num_layer, hidden_dim, head_num, ffn_dim=128,
                 att_dropout_prob=0.1, ffn_dropout_prob=0.1, type_of_layer='Pre-LN',
                 device='cuda:0'):
        super().__init__()
        if num_layer < 1:
    ...
    def forward(self, x_embd, x_embd_mask=None):
        ...
        for idx, layer_ in enumerate(self.transformer_layers):
            if self.type_of_layer == 'Pre-LN':
                ...
                x_ffn_1 = layer_[f'Dropout_{idx}_1'](x_ffn_1)
...
class Embedding_Numerical_Feature_Embedding(nn.Module):
    def __init__(self, token_dim, hidden_dim_size, num_of_layers=1, head_num=1, dropout_prob=0.1, init_weights=False,
                 norm=False, bias=False, dtype=torch.float64):
        super().__init__()
        self.device = self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    def forward(self, x_num):
        ...
        if self.type_of_layer == 'Pre-LN':
            # Execute Pre layer Norm
            x_norm = self.attention_layer_embedding_Transform[f'norm_layer_0_0'](x_embd)
            x_attn, x_attn_mask = self.attention_layer_embedding_Transform[f'attention_layer_0'](x_norm, x_norm, x_norm)
            x_attn = self.attention_layer_embedding_Transform[f'Dropout_0_0'](x_attn)

            # Fully Forward
            x_attn_norm = self.attention_layer_embedding_Transform[f'norm_layer_0_1'](x_attn)
            x_ffn_0 = self.attention_layer_embedding_Transform[f'linear_layer_0_0'](x_attn_norm)
            x_ffn_1 = self.attention_layer_embedding_Transform[f'linear_layer_0_1'](x_ffn_0)
            x_ffn_1 = self.attention_layer_embedding_Transform[f'Dropout_0_1'](x_ffn_1)
            encoded_embd = x_ffn_1
        else:
            # Execute Post layer Norm
