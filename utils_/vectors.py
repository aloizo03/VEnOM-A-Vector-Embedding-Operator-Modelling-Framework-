...
class Vectorise:
    ...
    def __init__(self, model_path, data_path, out_path):
    ...
    def save_vectors(self, dict_):
        file_name = os.path.join(self.output_path, f'vectors_{time.time()}.pickle')
        with open(file_name, 'wb') as handle:
            pickle.dump(dict_, handle, protocol=pickle.HIGHEST_PROTOCOL)

    def compute_vectors(self, batch_size):
        DataBuilder = self.dataset.get_Dataloader()
        dataloader = DataLoader(DataBuilder, batch_size=batch_size, shuffle=False)
