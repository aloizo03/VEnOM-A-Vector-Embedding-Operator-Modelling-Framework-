...
class Vectorise:
    ...
    def __init__(self, model_path, data_path, out_path, data_type, save_to, collection_name, d_token=100,
        ...
        elif self.data_type == 1 and self.tab_model == 'dataset2vec':
            # model_path is optional here: a Dataset2Vec checkpoint dir; when
            # None the pretrained split under dataset2vec/checkpoints is used
            self.embedder = Dataset2VecEmbedder(checkpoint_dir=model_path,
                                                split=d2v_split,
                                                n_batches=d2v_batches)
            self.dataset = Pipeline_Dataset(data_path=data_path, norm=False)
            self.vector_dim = d_token
        ...
        if self.save_to.lower() == 'local':
            if file_name is None:
                file_name = os.path.join(self.output_path,
                                         f'vectors_{self.collection_name}_{time.time()}_{self.vector_dim}.pickle')
                file_name_time_dict = os.path.join(self.output_path,
                                                   f'vectors_time_{self.collection_name}_{time.time()}_{self.vector_dim}.pickle')
                file_name_time_dict = os.path.join(self.output_path,
                                                   f'skecthes_{self.collection_name}_{time.time()}_{self.vector_dim}.pickle')
            ...
            if self.data_type == 1:
                if self.tab_model in ('tabfm', 'dataset2vec'):
    ...
    def compute_vectors(self, batch_size, num_of_workes=0, show_progress=True, ret_vec=False):
        ...
        if self.data_type == 1 and self.tab_model in ('tabfm', 'dataset2vec'):
            dataset_idxs = list(DataBuilder.idx_to_filename.keys())
            raw_vectors = []
            for idx in tqdm(dataset_idxs, desc=f'Extracting {self.tab_model} dataset vectors'):
                filepath = DataBuilder.idx_to_filename[idx]
                raw_vectors.append(self.embedder.embed_file(filepath))

            vectors_out_np = np.vstack(raw_vectors)
            self.vector_dim = vectors_out_np.shape[1]
            vectors_dataset_idx_np = np.asarray(dataset_idxs)
        elif self.data_type == 1:
